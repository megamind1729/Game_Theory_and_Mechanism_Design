...
def Gale_Shapley(suitor_prefs, reviewer_prefs) -> Dict[str, str]:
    ...
    suitor_prefs_edit = {}
    for suitor in suitor_prefs:
        suitor_prefs_edit[suitor] = suitor_prefs[suitor].copy()

    rev_matching = {}
    suitors = list(suitor_prefs_edit.keys())         # List of unmatched suitors
    while (len(suitors) > 0):
        curr_suitor = suitors.pop(0)
        top_reviewer = suitor_prefs_edit[curr_suitor].pop(0)    # Get the top (not yet rejected) preference of the suitor
